...
static int load_config(char *filename)
{
   ...
   if ((config_file = fopen(filename, "rb")))
   {
      ...
      while ((line = my_getline(config_file)))
      {
         ...
         do 
         {
            ...
            default:
               ...
               if (NULL == s)
               {
                  printf("load_config: missing '=' after key\n");
                  s = key + strlen(key);
               }
               else
               {
...

               {
                  ...
                  if (NULL == var)
                  {
                     printf("load_config: my_create failed\n");
                     return -1;
                  }
