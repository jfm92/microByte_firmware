...
SNSS_ReadBlockHeader (SnssBlockHeader *header, SNSS_FILE *snssFile)
{
   ...
   if (fread (headerBytes, 12, 1, fp) != 1)
...
SNSS_WriteBlockHeader (SnssBlockHeader *header, SNSS_FILE *snssFile)
{
   ...
   if (fwrite (headerBytes, 12, 1, fp) != 1)
...
SNSS_ReadFileHeader (SNSS_FILE *snssFile)
{
   ...
   if (fread (snssFile->headerBlock.tag, 4, 1, fp) != 1)
   ...
   if (fread (&snssFile->headerBlock.numberOfBlocks, sizeof (unsigned int), 1, fp) != 1)
...
SNSS_WriteFileHeader (SNSS_FILE *snssFile)
{
   ...
   if (fwrite (writeBuffer, 8, 1, fp) != 1)
...
SNSS_OpenFile (SNSS_FILE **snssFile, const char *filename, SNSS_OPEN_MODE mode)
{
   *snssFile = (SNSS_FILE *)malloc(sizeof(SNSS_FILE));
   ...
   {
     //abort();
      printf("Could not malloc SNSS File");
   ...
   if (SNSS_OPEN_READ == mode)
   {
     ...
     //printf("OpenFile read pos/size: %d/%d\n", fp.position(), fp.size());
   }
   else
   {
     ...
     (*snssFile)->headerBlock.numberOfBlocks = 0;
      //printf("OpenFile write pos/size: %d/%d\n", fp.position(), fp.size());
     if (!fp) {
...
SNSS_CloseFile (SNSS_FILE **snssFile)
{
  /* int prevLoc;
   ...
   printf("CloseFile 1 pos/size: %d/%d\n", fp.position(), fp.size());
...
SNSS_GetNextBlockType (SNSS_BLOCK_TYPE *blockType, SNSS_FILE *snssFile)
{
   ...
   if (fread (tagBuffer, TAG_LENGTH, 1, fp) != 1)
...
SNSS_SkipNextBlock (SNSS_FILE *snssFile)
{
   ...
   if (fseek (fp, TAG_LENGTH + sizeof (unsigned int), SEEK_CUR) != 0)
  // if (! fp.seekCur(TAG_LENGTH + sizeof (unsigned int)))
   ...
   if (fread (&blockLength, sizeof (unsigned int), 1, fp) != 1)
  // if (fp.read(&blockLength, 4) != 4)
   ...
   if (fseek (fp, blockLength, SEEK_CUR) != 0)
...
SNSS_ReadBaseBlock (SNSS_FILE *snssFile)
{
   ...
   if (fread (blockBytes, MIN (header.blockLength, BASE_BLOCK_LENGTH), 1, fp) != 1)
...
SNSS_WriteBaseBlock (SNSS_FILE *snssFile)
{
   ...
   if (fwrite (blockBytes, BASE_BLOCK_LENGTH, 1, fp) != 1)
...
SNSS_ReadVramBlock (SNSS_FILE *snssFile)
{
   ...
   if (fread (snssFile->vramBlock.vram, MIN (header.blockLength, VRAM_16K), 1, fp) != 1)
...
SNSS_WriteVramBlock (SNSS_FILE *snssFile)
{
   ...
   if (fwrite (snssFile->vramBlock.vram, snssFile->vramBlock.vramSize, 1, fp) != 1)
...
SNSS_ReadSramBlock (SNSS_FILE *snssFile)
{
   ...
   if (fread (&snssFile->sramBlock.sramEnabled, 1, 1, fp) != 1)
   ...
   if (fread (&snssFile->sramBlock.sram, MIN (header.blockLength - 1, SRAM_8K), 1, fp) != 1)
...
SNSS_WriteSramBlock (SNSS_FILE *snssFile)
{
   ...
   if (fwrite (&snssFile->sramBlock.sramEnabled, 1, 1, fp) != 1)
   //if (fp.write (&snssFile->sramBlock.sramEnabled, 1) != 1)
   {
      return SNSS_WRITE_FAILED;
   }
   
   if (fwrite (snssFile->sramBlock.sram, snssFile->sramBlock.sramSize, 1, fp) != 1)
...
SNSS_ReadMapperBlock (SNSS_FILE *snssFile)
{
   ...
   if (fread (blockBytes, MIN (0x8 + 0x10 + 0x80, header.blockLength), 1, fp) != 1)
...
SNSS_WriteMapperBlock (SNSS_FILE *snssFile)
{
   ...
   if (fwrite (blockBytes, MAPPER_BLOCK_LENGTH, 1, fp) != 1)
...
SNSS_ReadSoundBlock (SNSS_FILE *snssFile)
{
   ...
   if (fread (snssFile->soundBlock.soundRegisters, MIN (header.blockLength, 0x16), 1,fp) != 1)
...
SNSS_WriteSoundBlock (SNSS_FILE *snssFile)
{
   ...
   if (fwrite (snssFile->soundBlock.soundRegisters, SOUND_BLOCK_LENGTH, 1, fp) != 1)
